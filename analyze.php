...
if($file)
{
    ...
    while ($line = fgets($file))
    {
        ...
        if(preg_match('/->/', $line))
        {
            ...
            {
                $data['function'][$function]['this_arrow_equal'][] = array(
                    'text' => $arrow,
                    'line' => $line_count
                );
            }
            ...
            {
                $data['function'][$function]['this_arrow_call'][] = array(
                    'text' => $arrow,
                    'line' => $line_count
                );
            }
            ...
            {
                $data['function'][$function]['arrow'][] = array(
                    'text' => $arrow,
                    'line' => $line_count
                );
            }
        ...
        if(preg_match('/::/', $line))
        {
            ...
            if(preg_match('/::.*=/', $line))
            {
                $data['function'][$function]['static_equal'][] = array(
                    'text' => $static,
                    'line' => $line_count
                );
            }
            ...
            {
                $data['function'][$function]['static_call'][] = array(
                    'text' => $static,
                    'line' => $line_count
                );
            }
        ...
        {
                $data['function'][$function]['if_count'] += 1;
        }

        // loop
        if(preg_match('/for.*\(|foreach.*\(|while.*\(/', $line))
        {
                $data['function'][$function]['loop_count'] += 1;
        }
        ...
        if(!empty($data['function'][$function]))
        {
            $data['function'][$function]['nest'] = $nest;
        }
        ...
        {
            if(
                strpos( $line, ' '.$php_function.'(' ) !== false ||
                strpos( $line, ' '.$php_function.' (' ) !== false ||
                strpos( $line, '='.$php_function.'(' ) !== false ||
                strpos( $line, '='.$php_function.' (' ) !== false ||
                strpos( $line, '>'.$php_function.'(' ) !== false ||
            ...
            {
                // 改行の除去
                $php_func_name = str_replace("\n", '', $line);
                $data['function'][$function]['php_function_call'][] = array(
                    'keyword' => $php_function,
                    'text' => $php_func_name,
                    'line' => $line_count
                );
            }
...
$export_filepath = './'.$data['class'].'_analyze.tsv';
$contents = '';
$contents .= "class\tfunction\tphp_function_call\tmethod_call\tthis_method_call\tstatic_method_call\tmax_nest\treturn\tthrow\n";
foreach($data['function'] as $k => $v)
{
    $mc = isset($v['arrow']) ? count($v['arrow']): 0;
    $phpc = isset($v['php_function_call']) ? count($v['php_function_call']): 0;
    $this_mc = isset($v['this_arrow_call']) ? count($v['this_arrow']): 0;
    $static_mc = isset($v['static_call']) ? count($v['static_call']): 0;
    $return_cnt = isset($v['return']) ? count($v['return']): 0;
    $thorw_cnt = isset($v['throw']) ? count($v['throw']): 0;
    $contents .= "{$data['class']}\t{$v['name']}\t{$phpc}\t{$mc}\t{$this_mc}\t{$static_mc}\t{$v['nest']}\t{$return_cnt}\t{$thorw_cnt}\n";
}
